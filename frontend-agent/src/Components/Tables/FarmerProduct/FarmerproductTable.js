...
const FarmerproductTable = () => {
  ...
  const fetchAdminData = async () => {
    try {
      const response = await axios.get("https://finaldeploye.onrender.com/farmerproduct/"); // Replace with your actual backend API endpoint to fetch admin data
  ...
  const deletePro = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        axios
          .delete(`https://finaldeploye.onrender.com/farmerproduct/delete/${id}`)
          .then((response) => {
            fetchAdminData();
          });
      }
    ...
    },
    {
      ...
      selector: (row) => (
        <button
          ...
          onClick={() => handleShow(row._id, row.images, row.pName)}
        >
          View
        </button>
      ),
  ...
  const handleShow = (id, images, pName) => {
    setProdId(id);
    setPImage(images);
    setPName(pName);
    setShow(true);
          ...
          >
            ...
            <Modal.Body>
              ...
              >
                ...
                >
                  <img src={ppimg} style={{ width: "100%", height: "auto" }} />
                </div>
              </div>
            </Modal.Body>
          </Modal>
